...
module.exports = {
    ...
    async generateGiveaway(client, data, color, giveawayid) {
        ...
        if (giveaway && giveaway[0] && client && data) {
            let description = data.language.giveaway.replace(`{ggift}`, giveaway[0].cadeau).replace(`{gtime}`, `${this.getTime(giveaway[0].duration - (Date.now() - giveaway[0].datenow))}`).replace(`{ghost}`, `<@${giveaway[0].authorid}>`).replace(`{gwin}`, giveaway[0].gagnant) + "\n\n"
            ...
            if (giveaway[0].voice && giveaway[0].voice !== "0") {
                description = description + (data.language.react.voice).replace('{need}', client.functions.msToTime(giveaway[0].voice)) + '\n'
            }
